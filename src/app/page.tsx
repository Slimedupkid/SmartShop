...
import { CrawlerResult } from '../modules/crawlers/contracts/CrawlerResult';
...
export default function Home() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // We check if the orchestration overall succeeded or if it returned an array of domain errors
  ...
  return (
    <main style={{ padding: '2rem', fontFamily: 'system-ui, sans-serif', maxWidth: '800px', margin: '0 auto' }}>
      <h1>SmartShop MVP Extraction Test</h1>
      ...
      {/* 1. Error State */}
      {error && (
        <div style={{ padding: '1rem', backgroundColor: '#ffebee', color: '#cc0000', border: '1px solid #ffcdd2', borderRadius: '4px', marginBottom: '1rem' }}>
      ...
      {loading && (
        <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>
          Initializing headless request to Checkers WAF...
        </div>
      )}
      ...
      {result && result.data.length > 0 && !loading && (
        <div>
          <p style={{ marginBottom: '1rem', color: '#2e7d32', fontWeight: 'bold' }}>
            Successfully extracted {result.metadata.itemsFound} live products in {result.metadata.durationMs}ms
          </p>
          
          <div style={{ display: 'grid', gap: '1rem' }}>
            {result.data.map((product, index) => (
              <div key={index} style={{ padding: '1rem', border: '1px solid #e0e0e0', borderRadius: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.1rem' }}>{product.name}</h3>
                  <div style={{ color: '#666', fontSize: '0.9rem' }}>
                    <span style={{ marginRight: '1rem' }}>Retailer: <strong>{product.retailer}</strong></span>
